...
const node = document.getElementsByClassName("recipe")[0];
const clone = node.cloneNode(true);
node.remove();
recipeAssembler(recipes[getRandomArbitrary(1,recipes.length-1)]);
/*
author
isBasedOn
cookTime
...
function recipeAssembler(recipe){
    l_clone=clone.cloneNode(true);
document.getElementById("recipelist").appendChild(l_clone);
...
let url=recipe.url;
let last=document.getElementsByClassName("recipe").length-1
document.getElementsByClassName("title")[last].innerHTML= name;
document.getElementsByClassName("rating")[last].src=rating;
document.getElementsByClassName("tag")[last].innerHTML =tags;
document.getElementsByClassName("food")[last].src = image;
document.getElementsByClassName("description")[last].alt=description;
document.getElementsByClassName("recipe")[last].style.visibility = "visible";
//document.getElementsByClassName("url")[0].innerHTML = url;

//document.getElementsByClassName("recipe")[0].style.visibility = "hidden";

}

function recipeDestroyAll(){
while(document.getElementsByClassName("recipe").length !=0){
...
let search=document.getElementById("site-search").value;
    recipes.forEach(recipeSearch,search);
    
}

function recipeSearch(recipe,index){    
    //console.log(recipe.tags);
annoyingOnes=false;
recipe.tags.forEach(halfkeyword,this);
console.log(annoyingOnes);
//annoyingOnes=annoyingOnes||recipe.recipeInstructions.forEach(halfkeyword);
//annoyingOnes=annoyingOnes||recipe.recipeIngredient.forEach(halfkeyword);
if (
    //keyword(recipe.author,this)||
//keyword(recipe.isBasedOn,this)||
//keyword(recipe.cookTime,this)||
//keyword(recipe.datePublished,this)||
//keyword(recipe.prepTime,this)||
annoyingOnes||
//keyword(recipe.recipeYield,this)||
keyword(recipe.name,this)||
keyword(recipe.description,this)){
    console.log(index+" succeeded");
    recipeAssembler(recipes[index]);
    
}else{
    console.log(index+" Failed")
}
}
function keyword(check,keyword){
    let result = check.search(keyword);
    if(result != -1){
        return true;
    }else{
        return false
...
function halfkeyword(item,index){
console.log(this==item);
    if(item==this||annoyingOnes){
    annoyingOnes=item,this}
}